...
**Goal:** `./scripts/start.sh` should start IB Gateway automatically without the user needing to open it manually.
...
**Notes:**
- `SYMBOLS` list is already in `src/process/config.py` — easy to extend
...
**Goal:** The current `scripts/generate_report.py` is hardcoded to AAPL. Update it to generate a report for all monitored symbols using the live Parquet data.
